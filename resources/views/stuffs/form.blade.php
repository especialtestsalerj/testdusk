            ...
            <div class="card-header">
                <div class="row">
                    ...
                    <div class="col-sm-4 align-self-center d-flex justify-content-end">
                        ...
                        @include('partials.save-button', ['model'=>$stuff, 'backUrl' => 'routines.show', 'permission'=>($routine->status ? 'stuffs:update' : ''), 'id' =>$routine_id])
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="datetime-local" max="3000-01-01T23:59" class="form-control text-uppercase" name="entranced_at" id="entranced_at" value="{{is_null(old('occurred_at')) ? (formMode() == 'create' ? $routine->entranced_at->format('Y-m-d ').date('H:i') : $stuff->entranced_at_formatted) : old('occurred_at')}}" @disabled(!$routine->status)/>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="exited_at">Saída</label>
                            <input type="datetime-local" max="3000-01-01T23:59" class="form-control text-uppercase" name="exited_at" id="exited_at" value="{{is_null(old('exited_at')) ? $stuff->exited_at_formatted: old('exited_at')}}" @disabled(!$routine->status)/>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="description" id="description" @disabled(!$routine->status)>{{is_null(old('description')) ? $stuff->description: old('description')}}</textarea>
                        </div>
                    </div>
                </div>
            </div>
