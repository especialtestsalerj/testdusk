...
@section('content')
    <div class="card card-default mx-0 my-0 mx-lg-5 my-lg-4">
        <form name="formulario" id="formulario" @if(formMode() == 'show') action="{{ route('visitors.updateFromDashboard', ['routine_id' => $routine_id, 'id' => $visitor->id]) }}" @else action="{{ route('visitors.storeFromDashboard', ['routine_id' => $routine_id])}}" @endIf method="POST">
            @csrf
            ...
            <div class="card-header py-4 px-4">
                <div class="row">
                    <div class="col-sm-8 align-self-center">
                        <h4 class="mb-0">
                            <a href="{{ route('visitors.index', ['routine_id' => $routine_id]) }}">Visitantes</a>
                    ...
                    <div class="col-sm-4 align-self-center d-flex justify-content-end">
                        @include('partials.save-button', ['model'=>$visitor, 'backUrl' => 'visitors.index', 'permission'=>($routine->status && !request()->query('disabled') ? 'visitors:update' : ''), 'id' =>$routine_id])
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="datetime-local" max="3000-01-01T23:59" class="form-control text-uppercase" name="exited_at" id="exited_at" value="{{ is_null(old('exited_at')) ? $visitor->exited_at_formatted: old('exited_at') }}" @disabled(!$routine->status) @if(request()->query('disabled')) disabled @endif/>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="description" id="description" @disabled(!$routine->status) @if(request()->query('disabled')) disabled @endif>{{ is_null(old('description')) ? $visitor->description: old('description') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
